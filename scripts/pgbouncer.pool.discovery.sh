...
#poollist=$(psql -h $hostname -p $port -U $username -qAtX -F: --dbname=$dbname -c "show pools" |cut -d: -f1 |grep -v ^pgbouncer)
# get db + username, no pgbounce db or username allowed in pool list
poollist=$(psql -h $hostname -p $port -U $username -qAtX -F: --dbname=$dbname -c "show pools" |cut -d: -f1,2 |grep -v ^pgbounce| grep -v :pgbounce)
